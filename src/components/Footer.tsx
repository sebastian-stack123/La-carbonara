  ...
  return (
    <footer className="bg-carbonara-wine/20 border-t border-white/5 pt-16 pb-8 text-carbonara-ivory relative">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
        ...
        {/* Contact */}
        <div>
          <h3 className="font-sans text-[10px] uppercase tracking-widest text-carbonara-gold mb-6 font-bold">{t('footer.contact')}</h3>
        ...
        {/* Hours */}
        <div>
          <h3 className="font-sans text-[10px] uppercase tracking-widest text-carbonara-gold mb-6 font-bold">{t('footer.hours')}</h3>
          <ul className="space-y-3 font-light text-xs">
            <li className="flex justify-between opacity-80"><span>{t('footer.days1')}</span> <span>12:00 - 22:30</span></li>
            <li className="flex justify-between opacity-80"><span>{t('footer.days2')}</span> <span>12:00 - 23:00</span></li>
            <li className="flex justify-between text-carbonara-ivory opacity-90 font-medium items-center"><span>{t('footer.days3')}</span> <span className="text-right text-carbonara-gold text-[10px] leading-tight" dangerouslySetInnerHTML={{ __html: t('contact.hours3') }}></span></li>
            <li className="pt-2 border-t border-white/10 mt-2 opacity-80">{t('footer.execMenu')}</li>
          </ul>
        </div>

        {/* Links */}
        <div>
          <h3 className="font-sans text-[10px] uppercase tracking-widest text-carbonara-gold mb-6 font-bold">{t('footer.links')}</h3>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
        <div className="flex flex-col items-center md:items-start gap-4">
          <p className="text-[10px] opacity-70 uppercase tracking-widest flex flex-col gap-1">
            ...
            <span>{t('footer.admission')}</span>
          </p>
          <div className="flex flex-wrap justify-center md:justify-start gap-4 text-[10px] opacity-70 uppercase tracking-widest">
            <a href="#privacy" className="hover:opacity-100 hover:text-carbonara-gold transition-colors">{t('footer.privacy')}</a>
            <a href="#terms" className="hover:opacity-100 hover:text-carbonara-gold transition-colors">{t('footer.terms')}</a>
            <a href="#cookies" className="hover:opacity-100 hover:text-carbonara-gold transition-colors">{t('footer.cookies')}</a>
          </div>
        </div>
        <FlagHandshake className="scale-75 md:scale-90" />
        <div className="text-[10px] uppercase tracking-widest text-carbonara-gold">
          {t('footer.created')} <a href="https://mantaiweb.com/" target="_blank" rel="noopener noreferrer" className="underline hover:text-white transition-colors">MANTAI WEB DESIGNER COMPANY</a>
        </div>
      </div>
    </footer>
  );
